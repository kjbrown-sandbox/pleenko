...
- Emits: `coin_dropped`, `coin_landed(board_type, bucket_index, currency_type, amount, multiplier)`, `board_rebuilt`, `autodropper_adjust_requested`, `prestige_coin_landed`, `autodrop_failed(board_type)`, `row_upgrade_starting`, `row_upgrade_sweep_started(start_local_x, end_local_x, focus_local_y, sweep_duration)`.
...
- Two coloring modes: default reads `ThemeProvider.theme.background_color` and picks a darken/lighten nudge (menu); `use_theme_triangle_shades = true` short-circuits to a coin-flip between `theme.triangle_light_color` / `triangle_dark_color` (gameplay, palette-resolved). Per-triangle PEAK alpha is rolled at spawn in `[min_peak_alpha, max_peak_alpha]` and multiplied into the fade curve — defaults 1.0/1.0 (menu unchanged); gameplay backdrop sets 0.2/0.5 so dense overlap doesn't wash the background.
