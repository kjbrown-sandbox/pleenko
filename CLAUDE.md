...
- Owns every sound in the game: legacy SFX pools, procedural harp (multi-sample), procedural harp-long (10s decay variant for prestige), procedural arcade square-wave + kick, ambient pad per board, drone pool shared by bucket drones + harp sparkles, bucket/coin chimes, click sounds, drum pools (currently disabled).
...
- Methods: `play_bucket`, `play_peg_sparkle`, `play_peg_click`, `play_bucket_hit`, `set_active_board`, `on_coin_dropped`, `on_coin_landed`, `play_manual_drop_drum`, `play_autodropper_drum`, `play(sound_name, pitch, max_duration)`, `play_prestige`, `notify_autodropper_beat(interval)`, `should_sparkle(bt)`, `is_active_board(bt)`, `get_time_until_next_chord()`, `get_chord_duration()`, `get_chord_phase()`, `try_consume_bucket_activation(is_advanced)`, `force_play_bucket(board_type, bucket_idx, degree, is_advanced)`.
- `force_play_bucket` bypasses the queue and `BUCKET_WAIT` cooldown, calling `_play_bucket_now` directly. Used by the bucket-value upgrade ripple in `PlinkoBoard` which orchestrates its own timing (center-outward arpeggio at `BUCKET_WAIT / 2` intervals).
...
- Listens: `ThemeProvider.theme_changed` (update lofi bus effects + re-select AudioStyle), `ChallengeManager.challenge_state_changed` (re-select style), `ChallengeManager.tick` (phase-lock beat grid + fire kick; final 10s schedules a second kick +0.5s later via `create_timer`), `PrestigeManager.prestige_phase_changed` (fade drones/ambient on SLOW_MO, reset arpeggio on NONE).
- Prestige audio: on SLOW_MO entry, `_prestige_silencing` flag gates `request_bucket_play` and `force_play_bucket` to prevent audio bleed, and all drones + ambient are faded. At contact, `play_prestige()` plays a bass note (HarpLong, root × 0.25) + bell (3 octaves above) simultaneously, then drives a real-time ascending I maj7 arpeggio (`_tick_prestige_arpeggio` in `_process` using `Time.get_ticks_msec()`) at 0.125s intervals through 3 octaves of chord tones using HarpLong. Arpeggio notes ring for 10s (LINGERING drones, reclaimed when time_scale returns to 1.0).
...
- Methods: `setup(board_type)`, `request_drop(costs, coin_type, is_manual)`, `try_autodrop`, `add_two_rows`, `increase_bucket_values`, `flash_nearest_peg(coin_pos, currency_type)`, `get_bucket(index)`, `get_nearest_bucket(x)`, `mark_bucket_*` (target/hit/unhit/forbidden).
...
- On coin land: calls `AudioManager.request_bucket_play(board_type, bucket_idx, distance, is_advanced)` and `AudioManager.on_coin_landed`. Singing is suppressed while `_upgrade_animating` is true (the upgrade ripple owns the arpeggio). On peg contact: `flash_nearest_peg` calls `AudioManager.should_sparkle` and `play_peg_sparkle`, and fires peg VFX (flash + pulse always, halo always, ring on beat-armed sparkles in coin color).
- Bucket value upgrade ripple (`_play_bucket_value_upgrade_ripple`): when `increase_bucket_values` is called, instead of rebuilding the board, buckets are updated in-place with a center-outward ripple. Each distance group fires simultaneously: internal value is set immediately, `pulse_down` + ticket-counter label animation + `force_play_bucket` + `mark_singing`, then `pulse_up` after the press bottoms out. Groups are spaced at `BUCKET_WAIT / 2` intervals, creating a harp arpeggio. Rapid double-purchase kills the existing ripple tween and starts fresh.
...
- Methods: `setup(currency_type, position, value)`, `mark_hit`, `mark_target`, `mark_unhit`, `mark_forbidden`, `pulse`, `pulse_down`, `pulse_up`, `animate_value_upgrade(old_value, new_value, duration)`, `mark_singing`, `mark_stop_singing(duration)`.
