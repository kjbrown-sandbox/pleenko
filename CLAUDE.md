...
- Drone density control: drones are routed to a dedicated `Drones` audio bus (separate from `Melody` which carries cellos/chimes/kicks) with a compressor (threshold -18 dB, ratio 3, attack 10 ms, release 500 ms) followed by a small-room reverb (room 0.4, wet 0.2, hipass 0.2) — effect order matters, compressor tames the dry stack first, reverb blooms off the controlled signal. `MAX_ACTIVE_DRONES = 8` hard cap enforced at allocation via `_evict_oldest_drone_if_full` (priority: LINGERING → SPARKLE → ACTIVE, oldest-first tiebreak, fades evicted voice over `VisualTheme.eviction_fade_duration`). Per-voice exponential attenuation: voice N allocated while N-1 drones are already ringing plays at `VOICE_ATTENUATION_RATIO^(N-1)` amplitude (~2.5 dB/voice), making the first voice dominant and late voices settle into a bed rather than stacking into a wall. Compressor params are co-tuned with the attenuation curve — retune both together if either changes.
- Note: because drones live on the `Drones` bus, they are **not** affected by any effects on the `Melody` bus (such as the lofi low-pass filter). If lofi is re-enabled, mirror any Melody-bus effects onto the Drones bus too. The activation rate-limit (`try_consume_bucket_activation`, `ACTIVATION_RATE_DIVISOR = 4.0` → ~375 ms at default autodrop) gates **both** visual `mark_active` and audio `play_bucket` — tuning it affects gameplay responsiveness as well as audio pacing.
